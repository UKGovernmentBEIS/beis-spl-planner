...
describe('ShareTokenDecoder', () => {
  ...
  describe('#decode, version 1', () => {
    ...
    it('decodes salary information', () => {
      ...
      expect(result.secondary['salary-amount']).to.equal(
        expectedSecondarySalary
      )
      expect(result.secondary['salary-period']).to.equal(
        expectedSecondaryPeriod
      )
    })

    it('decodes weeks', () => {
      const encoded =
        '0!000!Yhkn!0!0!0!o8Y008YBh-_luy_FxN_Stlg8ggxiH4pC2Cp0000000m'
      const expectedPrimaryLeave = [
        '-11',
        '-10',
        '-9',
        '-8',
    ...
    it('decodes a complex data object', () => {
      const encoded =
        '0!kFx!81C2!14i3pD!1bfwB!E!o8Y8Y8YBh-_luy_lx__y_lxCxkxipCpCpCp0000000y'
      const expected = {
        'nature-of-parenthood': 'birth',
        'start-date-day': '02',
        'start-date-month': '10',
      ...
      expect(result['nature-of-parenthood']).to.equal(
        expected['nature-of-parenthood']
      )
      ...
      expect(result.primary['spl-eligible']).to.equal(
        expected.primary['spl-eligible']
      )
      expect(result.primary['shpp-eligible']).to.equal(
        expected.primary['shpp-eligible']
      )
      expect(result.primary['salary-amount']).to.equal(
        expected.primary['salary-amount']
      )
      expect(result.primary['salary-period']).to.equal(
        expected.primary['salary-period']
      )
      expect(result.secondary['spl-eligible']).to.equal(
        expected.secondary['spl-eligible']
      )
      expect(result.secondary['shpp-eligible']).to.equal(
        expected.secondary['shpp-eligible']
      )
      expect(result.secondary['salary-amount']).to.equal(
        expected.secondary['salary-amount']
      )
      expect(result.secondary['salary-period']).to.equal(
        expected.secondary['salary-period']
      )
    ...
    it('decodes another complex data object', () => {
      const encoded =
        '0!y3m!Yhkp!1vIs!1cz!D!pCpCpCpCm0CpCm0CpCp00pCp000000038Y8iY8Y0030'
      const expected = {
        'nature-of-parenthood': 'birth',
        primary: {
          'spl-eligible': 'yes',
